...
public class RPG_Game {
    ...
    public static void startGame() {
        ...
        Spitfire spitfire = new Spitfire("Drake", 250, 15); // Новый герой
    ...
    public static void playRound(Boss boss, Hero[] heroes) {
        ...
        for (Hero hero : heroes) {
            if (hero.getHealth() <= 0 && hero.isAlive()) {
                heroDied = true;
                hero.isAlive(); // Отмечаем, что герой мертв
            }
        }

        // Если кто-то из героев умер, Spitfire становится агрессивным
        for (Hero hero : heroes) {
            if (hero instanceof Spitfire && heroDied) {
                ((Spitfire) hero).triggerAggression();
            }
        }
